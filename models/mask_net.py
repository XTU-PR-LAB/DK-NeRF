...
class Blurring_Mask(nn.Module):

    def __init__(self, nb_ref_imgs=2, output_exp=False):
        super(Blurring_Mask, self).__init__()
    ...
    def forward(self, target_image, ref_imgs):
        ...
        ref_imgs = ref_imgs.view(target_image.shape) # 1, 3, Nrand, motion
        
        input = [target_image] 
        input.append(ref_imgs) 
        ...
        if self.training:
            return exp_mask1 # torch.Size([card, 2, 128, 1248])
        else:
            print('no this step')
            return exp_mask1 # torch.Size([card, 2, 128, 1248])
