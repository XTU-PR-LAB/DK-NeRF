...
HALF_PIX = 0.5
def soft_to_hard_mask(soft_mask, threshold=0.5):
    """
    Shifting softmask to rigidmask
    Args:
        soft_mask: a tensor, shape is (batch_size, height, width)
        threshold: defult setting value is 0.5
    Returns:
        rigid_mask, shape is (batch_size, height, width), values is 0 or 1
    """
    hard_mask = torch.where(soft_mask > threshold, torch.tensor(1.0), torch.tensor(0.0))
    return hard_mask

def init_linear_weights(m):     # Initialize the weights and biases of linear or transposed convolutional layers in neural network models
...
class Rigid_Blurring_Kernel(nn.Module):
    ...
    def rbk_warp(self, rays, r, v):
        r = r.reshape(r.shape[0], 3, self.num_motion) # r,v is rotation matrix and translation matrix
        v = v.reshape(v.shape[0], 3, self.num_motion)
        rays_o = rays[...,0] # Extract the origin and direction of the ray
        rays_d = rays[...,1]
        pts_rays_end = rays_o + rays_d # Calculate the endpoint of the ray
    ...
    def forward(self, rays, rays_info):
        # rays_info['images_idx']是所有射线所在图像编号size=(batch, 1)
        view_embedded = self.view_embedding_layer(rays_info['images_idx'].squeeze(-1)) # view_embedded (batch, 64)
...
class RBK_AWP(nn.Module):
    def __init__(self, num_img, view_embed_ch,
        ...
        self.view_embed_formask = View_Embedding(num_embed=num_img, embed_dim=(num_motion_rbk+1)*1*3)
        self.use_awp = use_awp
...
class NeRFAll(nn.Module):
    def __init__(self, args, blur_kernel_net=None):
        ...
        self.bk_mask_net = Blurring_Mask(nb_ref_imgs=1, output_exp=True)
        
        # print("=> using pre-trained weights for explainabilty and pose net")
        # weights = torch.load('./pretrained/exp_pose_model_best.pth.tar')
        # self.bk_mask_net.load_state_dict(weights['state_dict'], strict=False)

    def mlpforward(self, inputs, viewdirs, mlp, force_naive, inference, netchunk=1024 * 16): # 原为1024*64
    ...
    def forward(self, H, W, K, chunk=1024 * 32, use_rigidmask=False, rays=None, rays_info=None, poses=None, **kwargs):
        """
        ...
        if self.training:
            ...
            soft2rigidmask = True if use_rigidmask else False
            num_motion = self.mlp_rbk.num_motion + 1
            ...
            if self.blur_kernel_net is not None and not force_baseline and self.blur_model_type == 'dpnerf':
                ...
                if self.blur_kernel_net.use_dpnerf:
                    if self.use_awp:
                        ...
                        kwargs['use_awp'] = True
                        # generate rays mask
                        rays_map = rays.reshape(-1, num_motion, rays.shape[-2], rays.shape[-1]) # restoring dimension for flattened rays
                        rays_map = rays_map.sum(dim=-1).unsqueeze(-1)                        
                        rays_map = rays_map.permute(3, 2, 0, 1)
                        
                        view_embed_formask = self.blur_kernel_net.view_embed_formask(rays_info['images_idx'].squeeze(-1))
                        
                        bk_mask = self.bk_mask_net(rays_map, view_embed_formask)
                        bk_mask = bk_mask.squeeze().reshape(-1)

                        if soft2rigidmask:
                            bk_mask = soft_to_hard_mask(bk_mask, threshold=0.5)
                    else:
                    ...
                    if self.use_awp:    
                        ...
                        rgb = torch.mul(rgb, bk_mask.unsqueeze(-1))
    ...
    def render_path(self, H, W, K, chunk, render_poses, render_kwargs, render_factor=0):
        ...
        for i, c2w in enumerate(render_poses):
            ...
            print('render', i, ' TIME:', time.time() - t)
            t = time.time()
